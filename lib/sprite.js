...
module.exports = function (options) {
    ...
    return through2.obj(function (css, enc, callback) {
        var modRelativePath = path.dirname(css.relative);
        ...
        async.waterfall([
            ...
            function (cb) {
                ...
                if (!codelines || codelines.length === 0) {
                    ...
                    _this.push(new File({
                        base: opt.cssOut,
                        path: path.join(modRelativePath, cssBaseName + ".css"), //path.join(opt.cssOut, cssBaseName + ".css"),
                ...
                if (index === -1) {
                    ...
                    opt.spriteOut = opt.slicePath.substring(0, index + 1) + 'img/'
                }
                    ...
                    function (next) {
                        if (image1x) {
                            image1x.getBuffer(jimp.MIME_PNG, function (err, spriteBuffer) {
                                ...
                                _this.push(new File({
                                    base: '.', //opt.spriteOut,
                                    path: path.join(modRelativePath, opt.spriteOut, cssBaseName + ".png"), //path.join(opt.spriteOut, cssBaseName + '.png'),
                    ...
                    function (next) {
                        if (image2x) {
                            image2x.getBuffer(jimp.MIME_PNG, function (err, retinaSpriteBuffer) {
                                ...
                                _this.push(new File({
                                    base: '.', //opt.spriteOut,
                                    path: path.join(modRelativePath, opt.spriteOut, cssBaseName +  '@2x.png'),  //path.join(opt.spriteOut, cssBaseName + '@2x.png'),
                    ...
                    function (next) {
                        if (image3x) {
                            image3x.getBuffer(jimp.MIME_PNG, function (err, retinaSpriteBuffer) {
                                ...
                                _this.push(new File({
                                    base: '.', //opt.spriteOut,
                                    path: path.join(modRelativePath, opt.spriteOut, cssBaseName +  '@3x.png'), //path.join(opt.spriteOut, cssBaseName + '@3x.png'),
                                    contents: retinaSpriteBuffer
                                }))
